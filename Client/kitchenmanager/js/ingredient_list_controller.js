...
function getList(type = "All"){
    ...
    $.ajax({
        ...
        url: `http://localhost:8080/Server_war_exploded/KitchenManager/ingredients?type=${type}`,
...
function addingredient(){
    var image = document.getElementById("ingredient-insert-image").src,
     ...
     minimum = $("#form_min").val() ? $("#form_min").val() : "- - -" ,
     defaultv = $("#form_default").val(),
     maximum = $("#form_max").val() ?  $("#form_max").val() : "- - -" ;
     console.log(maximum);
     console.log("maximum");
     document.getElementById("ingredients-list").innerHTML += `<td><img class="icon-4" src="${image}"></td>
                         <td>${code}</td>
                         <td>${name}</td>
 ...
 function addingredienttoupdatetable(){
    var image = document.getElementById("ingredient-insert-image").src,
     option1 = document.getElementById("Ingredient-Name"),
     name = option1.options[option1.selectedIndex].text,
     code = $(option1.options[option1.selectedIndex]).attr("code");
     ...
     document.getElementById("ingredients-list").innerHTML += `<td><img class="icon-4" src="${image}"></td>
                         ...
                         <td>${name}</td>
                         <td style="display:none;">${ingID}</td>
                         <td><input type="text" value="${type}" style="width:fit-content" id ="ingtype" disabled></td>
                         <td> <input type="text" value="${minimum}"style="width:fit-content" id="min"  disabled>${unit}</td>
                         <td><input type="text" value="${defaultv}"style="width:fit-content" id ="default"  disabled> ${unit}</td>
                         <td><input type="text" value="${maximum}"style="width:fit-content" id="max"  disabled> ${unit}</td>
                         <td><button id="edit-${parseInt(this.ingID)}" class="btn tag-green">Edit</button></td>
     ...
     hidepop();
     changeImage();
     //triggers the update edit button function
 ...
 function submitDish(){
     ...
     $('#ingredients-list tr').each(function(row, tr) {
         ...
         console.log( parseInt($(tr).find('td:eq(7)').text()));
     });
     Ingredients.shift();
    ...
    var url = "http://localhost:8080/Server_war_exploded/KitchenManager/dish/add";
 ...
 function updateDish(){
    ...
    $('#ingredients-list tr').each(function(row, tr) {
        console.log(123)
        let min = parseInt($(tr).find('#min').val()) ;
        let def = parseInt($(tr).find('#default').val());
        let max = parseInt($(tr).find('#max').val());
        Ingredients[row] = {
            "ingID"     : parseInt($(tr).find('td:eq(3)').text()),
            // "type"      : $(tr).find('td:eq(4)').text(),
            "type"      : $(tr).find('#ingtype').val(),
            // "minimum"   : parseInt($(tr).find('td:eq(5)').text()),
            "minimum"   : min==NaN?0:min,
            // "defaultv"  : parseInt($(tr).find('td:eq(6)').text()),
            "defaultv"  : def==NaN?0:def,
            // "maximum"   : parseInt($(tr).find('td:eq(7)').text())
            "maximum"   : max==NaN?0:max
    ...
    var dishcode   = $("#dishcode").val(),
    dishname       = $("#dishname").val(),
    estimatedtime  = $("#estimatedTime").val()==null? 0 :$("#estimatedTime").val(),
    description    = $("#description").val(),
    image          = $("#image-label").attr("src");
   ...
   var xhr = new XMLHttpRequest();
   var url = "http://localhost:8080/Server_war_exploded/KitchenManager/dish/update";
   xhr.open("POST", url, true);
   var authHeader = "Bearer " + window.localStorage.getItem("jwt");
   xhr.setRequestHeader("authorization", authHeader);
   xhr.setRequestHeader("Content-Type", "application/json");
